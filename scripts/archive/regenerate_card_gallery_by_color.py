...
from scripts.utils.html_templates import save_gallery_html
...
def generate_html(card_cache):
    ...
    for name, data in card_cache.items():
        # Use centralized utility to determine color group
        color_group = get_color_group_for_card(data)
        
        # Handle lands specially
        type_line = data.get('type_line', '')
        if 'Land' in type_line and not data.get('colors'):
            color_group = 'lands'
        
        # Use centralized utility to determine rarity letter
        rarity_letter = get_rarity_letter(data.get('rarity', ''))
    ...
    for color_group in groups:
        for rarity_letter in groups[color_group]:
            groups[color_group][rarity_letter].sort(
                key=lambda x: (x.get('cmc', 0), x.get('name', ''))
            )
...
''')
            ...
            for card in cards:
                name = card.get('name', '')
                slugified_name = name.lower().replace(" ", "-").replace("/", "-").replace("'", "")
                local_image_path = card.get('local_image_path', f'../images/cards/{slugified_name}.png')
...
def main():
    ...
    # Write HTML file using centralized utility
    output_file = base_dir / "html_json" / "card-gallery.html"
    save_gallery_html(html_content, output_file)
